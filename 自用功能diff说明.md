...
- 适配官方 Claude/Gemini 原生测试请求格式，测试提示词仍使用 Vite 版本询问。
- 渠道测试失败/成功详情统一按固定字段顺序展示；非 200 响应也会带回脱敏后的请求与响应详情。
- 使用日志渠道操作中的优先级图标调整为排序图标。
...
- 合并官方最新代码，保留渠道测试 Vite 版本校验、测试详情展示、渠道复制与使用日志渠道操作等自用功能。
- 查看渠道密钥继续仅校验 Root 权限，不再要求 Passkey/2FA。
- 渠道测试弹窗不再自动聚焦端点类型。
- Vite 版本测试的 max tokens 调整为 1024。
