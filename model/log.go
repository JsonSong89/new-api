...
type Log struct {
	...
	ChannelPriority   int64  `json:"channel_priority" gorm:"-"`
	...
	if channelIds.Len() > 0 {
			...
			for _, channelId := range channelIds.Items() {
				if cacheChannel, err := CacheGetChannel(channelId); err == nil {
					channels = append(channels, struct {
						...
						Priority: cacheChannel.GetPriority(),
					})
				}
			}
		} else {
			// Bulk query channels from DB
			if err = DB.Table("channels").Select("id, name, status, priority").Where("id IN ?", channelIds.Items()).Find(&channels).Error; err != nil {
		...
		for i := range logs {
			...
			if ok {
				...
				logs[i].ChannelPriority = channel.priority
			}
		}
	}
